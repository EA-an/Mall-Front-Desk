...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      //需要置空name 去除面包屑  同时发请求 也需要清楚id
      //空字符串也会带过去给服务器 undefined不会带过去性能更好
      this.searchParams.categoryName = undefined;
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      //如果路由中有params参数 则不应该删除应该只删除query参数
    ...
    removeKeyword() {
      //同理 removeCategoryName
      this.searchParams.keyword = undefined;
      this.getData();
      //利用全局事件总线清楚input中的关键字
      this.$bus.$emit("clear");
      let location = { name: "search" };
      if (this.$route.query) {
    ...
    attrInfo(attr, attrValue) {
      ...
      //需要进行数组去重 可以使用 set 也可以使用index of
    ...
    //排序 需要知道 点的是那个 需要flag 需要与一开始的order做比较
    changeOrder(flag) {
